...
// Package module provides the internal modules
package module
...
func (db *Database) Connect(dsn types.DSN) error {
	var err error
...
func (db *Database) Migrate() bool {
	...
	db.Connection.AutoMigrate(&migration.Service{})
	db.Connection.AutoMigrate(&migration.Check{})
	db.Connection.AutoMigrate(&migration.Metric{})
	db.Connection.AutoMigrate(&migration.Alert{})
	db.Connection.AutoMigrate(&migration.Incident{})
	db.Connection.AutoMigrate(&migration.IncidentUpdate{})

	status = status && db.Connection.HasTable(&migration.Option{})
	status = status && db.Connection.HasTable(&migration.User{})
	status = status && db.Connection.HasTable(&migration.Service{})
	status = status && db.Connection.HasTable(&migration.Check{})
	status = status && db.Connection.HasTable(&migration.Metric{})
	status = status && db.Connection.HasTable(&migration.Alert{})
	status = status && db.Connection.HasTable(&migration.Incident{})
	status = status && db.Connection.HasTable(&migration.IncidentUpdate{})
...
func (db *Database) Rollback() bool {
	...
	db.Connection.DropTableIfExists(&migration.Service{})
	db.Connection.DropTableIfExists(&migration.Check{})
	db.Connection.DropTableIfExists(&migration.Metric{})
	db.Connection.DropTableIfExists(&migration.Alert{})
	db.Connection.DropTableIfExists(&migration.Incident{})
	db.Connection.DropTableIfExists(&migration.IncidentUpdate{})
